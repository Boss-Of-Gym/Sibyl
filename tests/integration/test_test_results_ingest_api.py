...
JWT_SIGNING_KEY = "test-signing-key-for-ingest"

REPORT_PAYLOAD = {
    "repository": "acme-ingest-test/widgets",
    "commitSha": "sha-ingest",
    "ciRunId": 7,
    "startedAt": "2026-07-02T00:00:00Z",
    "completedAt": "2026-07-02T00:01:00Z",
    "tests": [{"testIdentifier": "tests/test_x.py::test_x", "status": "passed"}],
}
...
async def test_ingest_returns_202_when_token_matches_the_resolved_installation(
    ...
    installation = await InstallationRepository().get_or_create_by_github_id(
        db_session, github_installation_id=555, organization_login="acme-ingest-test"
    )
    ...
    async with app.router.lifespan_context(app):
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.post(
                ...
                headers={
                    "Authorization": (
                        f"Bearer {_token(['write:test-results'], str(installation.id))}"
                    )
                },
            )
...
async def test_ingest_returns_403_when_token_has_no_installation_claim(
    ...
    await InstallationRepository().get_or_create_by_github_id(
        db_session, github_installation_id=556, organization_login="acme-ingest-test-2"
    )
    await db_session.commit()
    ...
    async with app.router.lifespan_context(app):
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.post(
                "/ingest/test-results",
                json={**REPORT_PAYLOAD, "repository": "acme-ingest-test-2/widgets"},
                headers={"Authorization": f"Bearer {_token(['write:test-results'])}"},
            )
...
async def test_ingest_returns_403_when_token_is_scoped_to_a_different_installation(
    ...
    await InstallationRepository().get_or_create_by_github_id(
        db_session, github_installation_id=557, organization_login="acme-ingest-test-3"
    )
    await db_session.commit()
    ...
    async with app.router.lifespan_context(app):
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            response = await client.post(
                "/ingest/test-results",
                json={**REPORT_PAYLOAD, "repository": "acme-ingest-test-3/widgets"},
                headers={
                    "Authorization": (
                        f"Bearer {_token(['write:test-results'], someone_elses_installation_id)}"
                    )
                },
            )
