...
| *(Addendum, launch-track Phase 1, 2026-07-07)* Golden-set eval harness (`eval/`) built with a synthetic, honestly-labeled dataset rather than deferred entirely until real usage data exists | Waiting for real production PR/failure data before building anything | The scoring logic and harness structure can be built and unit-tested now (`tests/unit/eval/test_scoring.py`); only the dataset's realism and the harness's live-API path are genuinely blocked on real usage — recorded explicitly as unverified in `eval/README.md` rather than either faked as "done" or left completely unstarted | Senior AI Engineer / Senior SDET |
| Cost/latency budget tests (from the LLM evaluation framework section above) explicitly not implemented in this pass | Fabricating placeholder "historical telemetry" numbers to satisfy the checklist | There is no real historical telemetry yet — nothing has been deployed. Same evidence-before-speculation principle Stage 10 applies; a fabricated baseline would be worse than an honestly absent one | Senior AI Engineer / user |
