...
| *(Addendum, launch-track Phase 1, 2026-07-07)* Metrics export is OTLP-push only (app → OTel Collector → Prometheus exporter); removed `infra/prometheus/prometheus.yml`'s direct-scrape `sibyl-api`/`sibyl-worker` jobs | Adding a second, direct-scrape Prometheus exposition endpoint (`opentelemetry-exporter-prometheus`) on both `api` and `worker` alongside the existing OTLP push | The direct-scrape jobs were configured against endpoints that were never actually implemented in code — `platform/observability.py` only ever wired OTLP push. Two parallel, redundant metrics-export mechanisms for the same data is unearned complexity at this project's scale; the SDK code already committed to OTLP, so the infra config was brought in line with it rather than the other way around. Also renamed the Helm chart's `worker.healthPort` (was `metricsPort`) since it now only ever serves `/healthz`/`/readyz`, never Prometheus-format metrics. | Staff Platform Engineer / user |
